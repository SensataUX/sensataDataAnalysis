...
createGraphData8optionEducM <- function(df = asData, 
                                        ...
                                        totalColumn = T){
  require(tidyverse)
  graphData <- df %>% select(originVar, educM, ponde) 
  graphData$y <- graphData[[originVar]]
  graphData <- na.omit(graphData)
  totalData <- graphData %>%
    summarise(val1 = prop.table(wtd.table(y, weights = ponde))[1]*100,
              ...
              val7 = prop.table(wtd.table(y, weights = ponde))[7]*100,
              val8 = prop.table(wtd.table(y, weights = ponde))[8]*100)
  totalData[["educM"]] <- factor("Total",
                                      levels = c(levels(df[["educM"]]),"Total"),
                                      labels = c(levels(df[["educM"]]),"Total"),
                                      ordered = T)
  totalData <- totalData %>%  pivot_longer(cols = starts_with("val"), 
                                           names_to = "Value", 
                                           names_prefix = "val",
  ...
  graphData$Value <- graphData$Value %>% factor(levels = c(1:8),
                                                labels = names(table(df[[originVar]])),
                                                ordered = T)
  graphData[["educM"]] <- graphData[["educM"]] %>% factor(levels = c(levels(df[["educM"]]),"Total"),
                                                                    labels = c(levels(df[["educM"]]),"Total"),
                                                                    ordered = T)
  if (totalColumn){
    graphData <- bind_rows(graphData, totalData)  
...
createGraphData8optionEducP <- function(df = asData, 
                                        ...
                                        totalColumn = T){
  require(tidyverse)
  graphData <- df %>% select(originVar, educ, ponde) 
  graphData$y <- graphData[[originVar]]
  graphData <- na.omit(graphData)
  totalData <- graphData %>%
    summarise(val1 = prop.table(wtd.table(y, weights = ponde))[1]*100,
              ...
              val7 = prop.table(wtd.table(y, weights = ponde))[7]*100,
              val8 = prop.table(wtd.table(y, weights = ponde))[8]*100)
  totalData[["educ"]] <- factor("Total",
                                      levels = c(levels(df[["educ"]]),"Total"),
                                      labels = c(levels(df[["educ"]]),"Total"),
                                      ordered = T)
  totalData <- totalData %>%  pivot_longer(cols = starts_with("val"), 
                                           names_to = "Value", 
                                           names_prefix = "val",
  ...
  graphData$Value <- graphData$Value %>% factor(levels = c(1:8),
                                                labels = names(table(df[[originVar]])),
                                                ordered = T)
  graphData[["educ"]] <- graphData[["educ"]] %>% factor(levels = c(levels(df[["educ"]]),"Total"),
                                                                    labels = c(levels(df[["educ"]]),"Total"),
                                                                    ordered = T)
  if (totalColumn){
    graphData <- bind_rows(graphData, totalData)  
...
createGraphData2OptionNHijos <- function(df = asData, 
                                       originVar,
                                       totalColumn = T){
  require(tidyverse)
  graphData <- df %>% select(all_of(originVar), q_EA_CA_02, ponde)
  graphData$y <- graphData[[originVar]]
  graphData <- na.omit(graphData)
  totalData <- graphData %>%
    summarise(val1 = prop.table(wtd.table(y, weights = ponde))[1]*100,
              val2 = prop.table(wtd.table(y, weights = ponde))[2]*100)
  totalData[["q_EA_CA_02"]] <- factor("Total",
                                levels = c(levels(df[["q_EA_CA_02"]]),"Total"),
                                labels = c(levels(df[["q_EA_CA_02"]]),"Total"))
  totalData <- totalData %>%  pivot_longer(cols = starts_with("val"), 
                                           names_to = "Value", 
                                           names_prefix = "val",
                                           values_to = "Porcentaje")
  ...
  graphData$Value <- graphData$Value %>% factor(levels = c(1:2),
                                                labels = names(table(df[[originVar]])),
                                                ordered = T)
  graphData[["q_EA_CA_02"]] <- graphData[["q_EA_CA_02"]] %>% factor(levels = c(names(table(df[["q_EA_CA_02"]])),"Total"),
                                                        labels = c(names(table(df[["q_EA_CA_02"]])),"Total"))
...
createGraphData5optionNH <- function(df = asData, 
                                      originVar,
                                      totalColumn = T){
  require(tidyverse)
  graphData <- df %>% select(originVar, q_EA_CA_02, ponde) 
  graphData$y <- graphData[[originVar]]
  graphData <- na.omit(graphData)
  totalData <- graphData %>%
    summarise(val1 = prop.table(wtd.table(y, weights = ponde))[1]*100,
...
createGraphData4OptionNH <- function(df = asData, 
                                     originVar,
                                     totalColumn = T){
  require(tidyverse)
  graphData <- df %>% select(originVar, q_EA_CA_02, ponde) 
...
createGraphData3OptionNH <- function(df = asData, 
                                     originVar,
                                     totalColumn = T){
  require(tidyverse)
  graphData <- df %>% select(originVar, q_EA_CA_02, ponde) 
